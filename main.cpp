...
void ShowSaved(Arguments *in, Reply *out);
RPCFunction rpcLED(&LEDControl, "LEDControl");
RPCFunction rpcSAVE(&ShowSaved, "ShowSaved");
int mode;
Thread t1, t2;
int angle_set = 30;
...
void publish_message_SET(MQTT::Client<MQTTNetwork, Countdown> *client)
{
    ...
    char buff[100];
    sprintf(buff, "QoS0 Hello, Python! #%d, thread_hold: %lf", message_num, thread_hold);
